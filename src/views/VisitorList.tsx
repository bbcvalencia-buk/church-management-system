            ...
            <div className="bg-white rounded-[24px] p-5 border border-gray-200 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="relative w-full md:max-w-md">
                    ...
                    <input
                        ...
                        className="w-full pl-11 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-none text-sm font-medium text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none"
                    />
                </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <div className="relative">
                        ...
                        <select
                            className="appearance-none bg-white border border-gray-200 text-sm font-bold text-gray-700 py-2.5 pl-10 pr-10 rounded-none cursor-pointer hover:bg-gray-50 transition-colors focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none"
                        >
                    ...
                    <label className="flex items-center gap-2 bg-white px-3 py-2.5 rounded-none border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors shrink-0">
                        <input
                            type="checkbox"
                            checked={showConverted}
                            onChange={(e) => setShowConverted(e.target.checked)}
                            className="rounded text-[var(--color-text-main)] focus:ring-blue-500/20 shadow-sm"
                        />
                        <span className="text-sm font-bold text-gray-700 whitespace-nowrap">Show Converted</span>
                    </label>

                    <button className="p-2.5 border border-gray-200 rounded-none hover:bg-gray-50 text-gray-600 transition-colors bg-white">
                        <Download size={18} />
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-[24px] overflow-hidden shadow-sm">
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="p-12 text-center text-gray-400 font-semibold">
                                        <div className="flex flex-col items-center gap-3">
                                            <div className="w-12 h-12 bg-gray-50 rounded-none flex items-center justify-center border border-gray-200">
                                ...
                                filteredVisitors.map((visitor) => {
                                    ...
                                    const getStatusColor = (status: string) => {
                                        switch (status) {
                                            case 'pending': return 'bg-gray-50 text-[var(--color-text-main)] border-blue-100';
                                    ...
                                    return (
                                        <tr key={visitor.id} className="border-b border-gray-50 hover:bg-gray-50/30 transition-colors group">
                                            <td className="p-5 pl-6">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-none bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-sm shrink-0">
                                            ...
                                            <td className="p-5">
                                                <span className={`px-2.5 py-1 rounded-none text-[11px] font-bold border flex items-center gap-1.5 w-fit ${getStatusColor(visitor.follow_up_status)}`}>
                                                    <span className={`w-1.5 h-1.5 rounded-none ${getStatusDot(visitor.follow_up_status)}`}></span>
                                            ...
                                            <td className="p-5 pr-6 text-right">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Link
                                                        ...
                                                        className="p-2 text-[var(--color-text-main)] bg-gray-50 hover:bg-blue-100 rounded-none transition-colors font-semibold text-xs flex items-center gap-1"
                                                    >
                                                        View Profile
                                                    </Link>
                                                    <button
                                                        onClick={() => setConfirmDelete({ isOpen: true, id: visitor.id, name: visitor.name })}
                                                        className="p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-none transition-colors"
                                                        title="Delete"
                                                    >
                                                        <Trash2 size={16} />
                                                    </button>
                ...
                <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                        <button className="p-2 border border-gray-200 rounded-none text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"><ChevronLeft size={16} /></button>
                        <button className="w-8 h-8 flex items-center justify-center border border-[var(--color-border)] bg-gray-50 text-[var(--color-text-main)] font-bold rounded-none text-sm">1</button>
                        <button className="p-2 border border-gray-200 rounded-none text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"><ChevronRight size={16} /></button>
                    </div>
                </div>
            </div>
            {/* Confirmation Modal */}
