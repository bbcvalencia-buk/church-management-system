  ...
  return (
    <div className="fixed inset-0 z-50 lg:left-64 flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px]" onClick={onClose}>
      <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-lg p-4 sm:p-6 relative animate-in zoom-in-95 duration-200 border border-gray-100 mx-2 sm:mx-0" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-900 p-2 bg-gray-100 hover:bg-gray-200 rounded-none transition-colors z-10" title="Close (Esc)">
        ...
        <div className="space-y-2 max-h-[45vh] overflow-y-auto pr-1 custom-scrollbar">
          {filteredAllMembers.map((m) => (
            <button key={m.id} onClick={(e) => { e.preventDefault(); onClose(); handleViewMember({ ...m, specific_role: m.member_number ? 'Member' : 'Member' }); }} className="w-full flex items-center justify-between p-3 border border-gray-100 rounded-none hover:bg-gray-50 transition-colors text-left">
              <div className="flex items-center gap-3">
                {m.profile_picture_url ? (
                  <img src={m.profile_picture_url} alt={`${m.first_name} ${m.surname}`} className="w-9 h-9 rounded-none object-cover shadow-sm bg-white" />
                ) : (
                  <div className="w-9 h-9 rounded-none bg-gray-100 border border-gray-200 flex items-center justify-center">
              ...
              <Eye size={15} className="text-[var(--color-text-main)]" />
            </button>
          ))}
          {filteredAllMembers.length === 0 && (
            <div className="text-center py-6 text-gray-500 text-sm font-semibold border-2 border-dashed border-gray-100 rounded-none">
